...
void ServerConnection::update(Map& map) {
	...
	// 	std::string received((char *) packet->data + 1);
	// 	latest = deserializeGameState(received);

	std::vector<Elf> elves;
	elves.push_back((Elf) {1, 0, 0, 0, 0});
	latest.elves = elves;
	latest.felhound = (Felhound) {2, 2, 0, 0};
	DEBUG(gameStateToString(latest));
	map.setGameState(latest);
	// game.getMap().setGameState(latest);
