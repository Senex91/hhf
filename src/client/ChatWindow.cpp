...
bool ChatWindow::initialiseSample() {
	using namespace CEGUI;
	try
	{
		// Retrieve the window manager
		WindowManager& winMgr = WindowManager::getSingleton();
		if(!FontManager::getSingleton().isDefined("Commonwealth-10"))
		...
		// Set the GUI Sheet
			...
			historySize->subscribeEvent(Editbox::EventTextAccepted,	Event::Subscriber(&ChatWindow::Event_HistorySizeChange, this)); 
			
			// Configure the text Editbox
			// Pressing <ENTER> puts the text into the history Listbox
			chatText->subscribeEvent(Editbox::EventTextAccepted,	Event::Subscriber(&ChatWindow::Event_ChatTextAdded,	this)); 
			
			// Configure the font name Combobox
			// Selecting a name changes the font used in the history Listbox and the text Editbox
			fontName->subscribeEvent(Combobox::EventTextChanged,	Event::Subscriber(&ChatWindow::Event_FontChange,	this)); 
			
			// Configure the font size Spinner
			// Selecting a size changes the font size used in the history Listbox and the text Editbox
			fontSize->subscribeEvent(Spinner::EventValueChanged,	Event::Subscriber(&ChatWindow::Event_FontChange,	this)); 
...
void ChatWindow::cleanupSample(void) {
	
}

bool ChatWindow::Event_HistorySizeChange(const CEGUI::EventArgs& args) {
	using namespace CEGUI;
...
bool ChatWindow::Event_ChatTextAdded(const CEGUI::EventArgs& args) {
	using namespace CEGUI;
...
bool ChatWindow::Event_FontChange(const CEGUI::EventArgs& args) {
	using namespace CEGUI;
	WindowManager& winMgr = WindowManager::getSingleton();
	Window* fontName = winMgr.getWindow("/ChatBox/FontName");
...
void ChatWindow::setHistorySize(const size_t& pSize) {
	using namespace CEGUI;
...
void ChatWindow::addChatText(const CEGUI::String& pText) {
	using namespace CEGUI;
	...
	if(pText.size())
	{
		...
		if(chatHistory->getItemCount() == mHistorySize)
		{
			/* We have reached the capacity of the Listbox so re-use the first Listbox item.
				T his code is a* little crafty.  By default the ListboxTextItem is created with
...
void ChatWindow::registerFont(const CEGUI::String& pLogicalName, const CEGUI::String& pFileName) {
	using namespace CEGUI;
	...
	if(mFontList.find(pLogicalName) == mFontList.end())
	{
		...
		if(mFontList.size() != 0
			&& FontManager::getSingleton().isDefined(testFont))
		{
			// Since this was only a test font we destroy it
			FontManager::getSingleton().destroy(testFont);
		}
		if(!font)
		{
			// This font is invalid
			if(FontManager::getSingleton().isDefined(testFont))
				return;
			else
				return;
		}
...
CEGUI::Font* ChatWindow::makeFont(const CEGUI::String& pFontName, const CEGUI::String& pFileName, const CEGUI::String& pSize) {
	using namespace CEGUI;
	
	Font* font;
	try
	{
		if(FontManager::getSingleton().isDefined(pFontName))
		{
			...
			font = &FontManager::getSingleton().get(pFontName);
		...
		else
		{
			...
			font = &FontManager::getSingleton().createFreeTypeFont(pFontName,10, true, pFileName); //TODO: do this properly
		}
		//font->load(); //TODO - do i need to replace this with something?
	}
	catch(Exception& e)
	{
...
void ChatWindow::changeFont(const CEGUI::String& pFontLogicalName, const CEGUI::String& pFontSize) {
	using namespace CEGUI;
	WindowManager& winMgr = WindowManager::getSingleton();
	
	if(!FontManager::getSingleton().isDefined(mChatFontName))
	{
	...
	/* Alter the area of the Editbox.  The original value is {{0.01,0},{1,-30},{0.99,0},{1,-5}}
		T he value we are* altering is the "-30" within the second couplet, defining the position of
	...
	/* Alther the area of the Listbox.  Here we only need the lower y coordinate.  Since this
		v alue is the sam*e as the upper y coordinate of the Editbox we do not need to calculate
